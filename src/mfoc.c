...
void mf_inizia(mftag *t, mfreader *r);
...
int main(int argc, char * const argv[]) {
	...
	// Settings value for zsleep fuction
...
//#include "keys.h"  //libreria non so se serve
//		{0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
//		{0xb5, 0xff, 0x67, 0xcb, 0xa9, 0x51},
//		{0x67, 0xa6, 0x15, 0xc4, 0x9e, 0xa6}
...
//		{0x8f, 0xd0, 0xa4, 0xf2, 0x56, 0xe9}
	...
	mf_inizia(&t, &r);
	nfc_initiator_init(r.pdi);
	// Drop the field for a while, so can be reset
	nfc_configure(r.pdi,NP_ACTIVATE_FIELD,true);
	// Let the reader only try once to find a tag
	nfc_configure(r.pdi,NP_INFINITE_SELECT,false);
	// Configure the CRC and Parity settings
	nfc_configure(r.pdi,NP_HANDLE_CRC,true);
	nfc_configure(r.pdi,NP_HANDLE_PARITY,true);
	// wait for tag to appear
	for (i=0;!nfc_initiator_select_passive_target(r.pdi, nm, NULL, 0, &t.nt) && i < 10; i++) zsleep();

//	mf_select_tag(r.pdi, &(t.nt));
	
	// Save tag authuid and info about block size (b4K)
	t.b4K = (t.nt.nti.nai.abtAtqa[1] == 0x02);
	t.authuid = (uint32_t) bytes_to_num(t.nt.nti.nai.abtUid, 4);
	...
	fprintf(stdout, "Found MIFARE Classic %cK card with authuid: %08x\n", (t.b4K ? '4' : '1'), t.authuid);
	
	// Try to authenticate to all sectors with default keys
	// Set the authentication information (u.id)
	...
	nfc_configure(r.pdi, NP_HANDLE_CRC, true);
	nfc_configure(r.pdi, ND_HANDLE_PARITY, true);
...
void mf_anticollision(mftag t, mfreader r) {
	...
	if (!nfc_initiator_select_passive_target(r.pdi, nm, NULL, 0, &t.nt)) {
		fprintf(stderr, "\n\n!Error: tag has been removed in the dreaded mf_anticollision\n");
		exit(1);
	}
}
...
int mf_enhanced_auth(int e_sector, int a_sector, mftag t, mfreader r, denonce *d, pKeys *pk, char mode, bool dumpKeysA) {
	...
	// fprintf(stdout, "\nAuth command:\t");
	// print_hex(Auth, 4);
//X	nfc_configure(r.pdi,NP_ACTIVATE_FIELD,true);

	while (!nfc_initiator_select_passive_target(r.pdi, nm, NULL, 0, &t.nt)) {
		printf("_");
		zsleep();
//exit(1);
	}
...
//	if (!nfc_configure(r.pdi, ND_HANDLE_PARITY, true))  {
//		nfc_perror (r.pdi, "nfc_configure parity");
//		exit (EXIT_FAILURE);
//	}	// First, we do not want to send arbitrary parity bits
	
	// We need full control over the CRC
//	for (i=0;!nfc_initiator_select_passive_target(r.pdi, nm, NULL, 0, &t.nt) && i < 10; i++) zsleep();
	if (!nfc_configure(r.pdi, NP_HANDLE_CRC, false))  {
	...
	// print_hex(Rx, 4);
//	printf("Config ready xcpt parity.\n");fflush(stdout);
	...
	// Load (plain) authuid^nt into the cipher {48..79} bits
	...
	if (!nfc_configure(r.pdi, ND_HANDLE_PARITY, false)) {
		nfc_perror (r.pdi, "nfc_configure parity");
		exit (EXIT_FAILURE);
	}
	...
	if ((!nfc_initiator_transceive_bits(r.pdi, ArEnc, 64, ArEncPar, Rx, &RxLen, RxPar)) || (RxLen != 32)) {
		fprintf(stderr, "Reader-answer transfer error 1, exiting..\n");
		exit(1);
	}
	...
	if (mode == 'd') {
		for (m = 0; m < d->num_distances; m++) {
			...
			if ((!nfc_initiator_transceive_bits(r.pdi, ArEnc, 64, ArEncPar, Rx, &RxLen, RxPar)) || (RxLen != 32)) {
				fprintf(stderr, "Reader-answer transfer error 2, exiting..\n");
				exit(1);
			}
			Nt = prng_successor(Nt, 32);
			if (!((crypto1_word(pcs, 0x00, 0) ^ bytes_to_num(Rx, 4)) == (Nt&0xFFFFFFFF))) {
	...
	if (mode == 'r') {
		...
		if (!nfc_initiator_transceive_bits(r.pdi, AuthEnc, 32, AuthEncPar,Rx, &RxLen, RxPar)) {
			...
			if (!nfc_configure(r.pdi, NP_HANDLE_CRC, true))  {
	...
	for (i=0;!nfc_configure(r.pdi, ND_HANDLE_PARITY, true) && i < 10; i++)  {
		zsleep();
		printf("Parity");
	}
	if (i==10) { fprintf(stderr,"10 config errors, aborting now.\n"); exit (EXIT_FAILURE); }

	for (i=0;!nfc_configure(r.pdi, NP_HANDLE_CRC, true) && i < 10; i++) {
		zsleep();
		printf("CRC");
	}
	if (i==10) { fprintf(stderr,"10 config errors, aborting now.\n"); exit (EXIT_FAILURE); }
